...
string CBase64::Encode(string data)
{
	string s;
	Encode((const unsigned char*)(data.c_str()),(unsigned long)data.length(),s);
	return s;
}

string CBase64::Decode(string data)
{
	string s;
...
int CBase64::GetDecodeBufferLength(string data)
{
	int len = 0;
	len = (data.length() >> 2) * 3-2;
	return len;
}

string CBase64::GBToUTF8(const char* str)
{
	std::string result;
	WCHAR *strSrc;
	TCHAR *szRes;
...
string CBase64::UTF8ToGB(const char* str)
{
	std::string result;
	WCHAR *strSrc;
	TCHAR *szRes;
